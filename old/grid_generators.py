...
def l_shape_with_hole(grid_size=30, spacing=5.0, hole_radius_cells=3):
    ...
    """
    gm = GridManager.l_shape(grid_size, spacing)
...
def urban_canyon(width_cells, length_cells, spacing=5.0):
    ...
    """
    gm = GridManager(file_path=None, grid_spacing=spacing,
...
def open_grid(grid_size=30, spacing=5.0):
    ...
    return GridManager(file_path=None, grid_spacing=spacing,
                       default_grid_size=grid_size)
...
def matrix_grid(file_path, spacing=5.0):
    """
    Load a GridManager from an Excel (.xlsx) file.
    ...
    """
    return GridManager(file_path=file_path, grid_spacing=spacing)
...
def make_grid(sc, matrix_path=None, force_matrix=False):
    """
    Build a GridManager from a scenario dict.
    ...
    if domain == "open":
        ...
        return GridManager.l_shape(size, spacing)
    elif domain == "L_hole":
