...
class LevelsCallback(Callback):
    ...
    def notify(self, algorithm):
        ...
        if len(feas_idx) > 0:
            idx = feas_idx[knee_point(F[feas_idx])]
        else:
...
def print_metrics(label, metrics):
    ...
    for k, v in metrics.items():
        print(f"  {k:<28}: {v if isinstance(v, int) else f'{v:.6g}'}")
    print(sep)
...
def save_metrics_txt(metrics, sc, filename, coords=None):
    ...
    with open(filename, "w") as f:
        ...
        for k, v in metrics.items():
            val_str = str(v) if isinstance(v, int) else f"{v:.6g}"
...
def run_scenario(sc, args, out_dir):
    ...
    if getattr(args, "levels", False):
        ...
        cb = LevelsCallback(levels_dir, gm, sc)
    ...
    plot_pareto(pareto_F, knee_idx=knee_point(pareto_F), filename=pareto_png)
...
def main():
    ...
    parser.add_argument("--kernel",   type=str,   default=None, choices=["bessel", "hankel"],
                        help="Override scenario kernel (bessel or hankel)")
    # optimizer
