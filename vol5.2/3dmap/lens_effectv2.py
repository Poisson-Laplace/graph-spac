...
def compute_banana_doughnut_kernel(x, y, z, p1, p2, freq=2.0, vs=500.0, focus=30.0):
    ...
    # Prevent sigma from artificially dragging the focal spot into the abyss
    sigma = 0.5 * lam * (abs(z + focus) / focus + 1.0)
    sensitivity = np.exp(-(delta_L**2) / (2 * sigma**2))
    
    doughnut_hole = 1.0 - np.exp(-(z**2) / (0.1 * lam**2))
    
    # Tighten the Gaussian anchor (use focus/4.0 instead of focus/2.0)
    depth_focus = np.exp(-((z + focus)**2) / (2 * (focus / 4.0)**2)) 
...
def plot_3d_isosurface(coords, sc, gm, out_dir):
    ...
    # 3D Grid definition (High Resolution 100j)
    X, Y, Z = np.mgrid[0:gm.x_max:100j, 0:gm.y_max:100j, 0:-60:100j]
    V = np.zeros_like(X)
    ...
    while threshold > 0.1:
        try:
            verts, faces, normals, values = measure.marching_cubes(V, level=threshold, spacing=(gm.x_max/100, gm.y_max/100, 60/100))
            verts[:, 2] = -verts[:, 2] 
    ...
    views = [
        {"name": "perspective", "elev": view_elev, "azim": view_azim},
        {"name": "top_down_xy", "elev": 90, "azim": -90},
        {"name": "side_xz", "elev": 0, "azim": -90},
        {"name": "front_yz", "elev": 0, "azim": 0}
    ]
    
    for v in views:
        ax.view_init(elev=v["elev"], azim=v["azim"])
        
        # Reset all labels to their proper text
        ax.set_xlabel("East-West Coordinate [m]", color='black', labelpad=10)
        ax.set_ylabel("North-South Coordinate [m]", color='black', labelpad=10)
        ax.set_zlabel("Depth [m]", color='black', labelpad=10)
        ...
        # Hide the axis that is perpendicular to the screen seamlessly
        if v["name"] == "top_down_xy":
            ax.set_zlabel("")
            ax.tick_params(axis='z', labelcolor='none')
        elif v["name"] == "side_xz":
            ax.set_ylabel("")
            ax.tick_params(axis='y', labelcolor='none')
        elif v["name"] == "front_yz":
            ax.set_xlabel("")
            ax.tick_params(axis='x', labelcolor='none')
            
        out_file = os.path.join(out_dir, f"sc{sc_id}_3D_{mode_name}_{v['name']}.png")
        plt.tight_layout()
        plt.savefig(out_file, dpi=400, facecolor='white', edgecolor='none')
        print(f"[3D Map] Successfully rendered {v['name']} view to -> {out_file}")
